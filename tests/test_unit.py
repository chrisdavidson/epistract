...
# === Phase 6: db_corpus tests (DB-01..DB-05, T-6-01) ===


@pytest.mark.unit
def test_db_corpus_sqlite(tmp_path):
    """DB-01: fetch_sqlite_rows + serialize_row write 4 files from a 5-row fixture.

    Row 4 (all-NULL) must be skipped per DB-03. Expected filenames are
    patients_1.txt, patients_2.txt, patients_3.txt, patients_5.txt.
    """
    ...
    for row in rows:
        fname = write_db_corpus.serialize_row(
            ...
            table_name="patients",
            pk_col="id",
            db_url_redacted="sqlite:///tests/fixtures/sample.sqlite",
            docs_dir=docs_dir,
        )
        if fname:
            written.append(fname)
    assert len(written) == 4, f"expected 4 written, got {len(written)}: {written}"
    assert {"patients_1.txt", "patients_2.txt", "patients_3.txt", "patients_5.txt"} == set(written)
    contents = (docs_dir / "patients_1.txt").read_text(encoding="utf-8")
    assert "Hypertension" in contents
    assert "alice" not in contents and "s3cret" not in contents  # T-6-01 negative check


@pytest.mark.unit
def test_db_corpus_format(tmp_path):
    """DB-02: serialize_row produces header lines in exact documented order.

    Line 0: 'Source: ', line 1: 'Database: ', line 2: 'Table: ',
    line 3: 'Row_ID: ', then 'Field_<col>:' lines, then blank line,
    then 'CONTENT:'.
    """
    write_db_corpus = pytest.importorskip("write_db_corpus")
    ...
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    row = {"id": 7, "name": "Acme Corp", "notes": "Vendor since 2019."}
    fname = write_db_corpus.serialize_row(
        row=row,
        table_name="vendors",
        pk_col="id",
        db_url_redacted="sqlite:///synthetic.db",
        docs_dir=docs_dir,
    )
    assert fname is not None, "serialize_row returned None for a non-empty row"
    lines = (docs_dir / fname).read_text(encoding="utf-8").splitlines()
    assert lines[0].startswith("Source: "), f"line 0 must start 'Source: ', got: {lines[0]!r}"
    assert lines[1].startswith("Database: "), f"line 1 must start 'Database: ', got: {lines[1]!r}"
    assert lines[2].startswith("Table: "), f"line 2 must start 'Table: ', got: {lines[2]!r}"
    assert lines[3].startswith("Row_ID: "), f"line 3 must start 'Row_ID: ', got: {lines[3]!r}"
    # Field_ lines must appear for every non-PK column
    field_lines = [ln for ln in lines[4:] if ln.startswith("Field_")]
    assert len(field_lines) >= 1, f"expected at least one Field_ line, got: {lines!r}"
    # Blank line then CONTENT:
    try:
        blank_idx = lines.index("")
    except ValueError:
        raise AssertionError(f"no blank line separating header from CONTENT in: {lines!r}")
    assert lines[blank_idx + 1] == "CONTENT:", (
        f"line after blank must be 'CONTENT:', got: {lines[blank_idx + 1]!r}"
    )


@pytest.mark.unit
def test_db_corpus_skip_empty(tmp_path):
    """DB-03: serialize_row returns None and writes no file when all non-PK values are None/empty."""
    write_db_corpus = pytest.importorskip("write_db_corpus")

    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    result = write_db_corpus.serialize_row(
        row={"id": 99, "name": None, "notes": ""},
        table_name="patients",
        pk_col="id",
        db_url_redacted="sqlite:///synthetic.db",
        docs_dir=docs_dir,
    )
    assert result is None, f"expected None for empty-content row, got: {result!r}"
    assert list(docs_dir.iterdir()) == [], "no file should be written for an empty-content row"


@pytest.mark.unit
def test_db_corpus_filename(tmp_path):
    """DB-04: filename is '{table_name}_{pk_val}.txt'; special chars in table_name become '_'."""
    write_db_corpus = pytest.importorskip("write_db_corpus")

    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    # Two different tables, same pk_val — both files must coexist
    fname_p = write_db_corpus.serialize_row(
        row={"id": 1, "name": "Alice Patient", "notes": "Some notes."},
        table_name="patients",
        pk_col="id",
        db_url_redacted="sqlite:///synthetic.db",
        docs_dir=docs_dir,
    )
    fname_v = write_db_corpus.serialize_row(
        row={"id": 1, "vendor": "Acme Corp", "contract": "Main contract."},
        table_name="vendors",
        pk_col="id",
        db_url_redacted="sqlite:///synthetic.db",
        docs_dir=docs_dir,
    )
    assert fname_p == "patients_1.txt", f"expected patients_1.txt, got: {fname_p!r}"
    assert fname_v == "vendors_1.txt", f"expected vendors_1.txt, got: {fname_v!r}"
    assert (docs_dir / "patients_1.txt").exists(), "patients_1.txt must exist"
    assert (docs_dir / "vendors_1.txt").exists(), "vendors_1.txt must exist"
    # Special chars in table_name must be sanitized to underscores
    fname_s = write_db_corpus.serialize_row(
        row={"id": 2, "notes": "Special char table name test."},
        table_name="with spaces!",
        pk_col="id",
        db_url_redacted="sqlite:///synthetic.db",
        docs_dir=docs_dir,
    )
    assert fname_s is not None, "serialize_row returned None for non-empty row with special table name"
    stem = Path(fname_s).stem
    assert stem.startswith("with_spaces_"), (
        f"special chars in table_name must become underscores; got stem: {stem!r}"
    )


@pytest.mark.unit
def test_db_corpus_missing_driver(monkeypatch):
    """DB-05: _require_sqlalchemy/_require_neo4j raise ImportError with 'uv pip install' hint."""
    write_db_corpus = pytest.importorskip("write_db_corpus")

    monkeypatch.setattr(write_db_corpus, "HAS_SQLALCHEMY", False)
    with pytest.raises(ImportError, match=r"uv pip install.*sqlalchemy"):
        write_db_corpus._require_sqlalchemy()

    monkeypatch.setattr(write_db_corpus, "HAS_NEO4J", False)
    with pytest.raises(ImportError, match=r"uv pip install.*neo4j"):
        write_db_corpus._require_neo4j()


@pytest.mark.unit
def test_db_corpus_redact_url():
    """T-6-01: _redact_url strips user:password from connection URLs before any .txt write."""
    write_db_corpus = pytest.importorskip("write_db_corpus")

    # PostgreSQL URL with credentials — user and password must be absent from output
    result_pg = write_db_corpus._redact_url("postgresql+psycopg2://alice:s3cret@h/db")
    assert "alice" not in result_pg, f"user must be redacted; got: {result_pg!r}"
    assert "s3cret" not in result_pg, f"password must be redacted; got: {result_pg!r}"
    assert "h" in result_pg, f"host must be preserved; got: {result_pg!r}"
    assert "db" in result_pg, f"database must be preserved; got: {result_pg!r}"

    # Bolt URL — neo4j:hunter2 must be stripped, localhost:7687 preserved
    result_bolt = write_db_corpus._redact_url("bolt://neo4j:hunter2@localhost:7687")
    assert "neo4j:hunter2" not in result_bolt, f"creds must be redacted; got: {result_bolt!r}"
    assert "localhost" in result_bolt, f"host must be preserved; got: {result_bolt!r}"
    assert "7687" in result_bolt, f"port must be preserved; got: {result_bolt!r}"

    # SQLite URL — no creds present; must be returned unchanged
    sqlite_url = "sqlite:///./local.db"
    result_sqlite = write_db_corpus._redact_url(sqlite_url)
    assert result_sqlite == sqlite_url, (
        f"sqlite URL with no creds must be unchanged; got: {result_sqlite!r}"
    )
