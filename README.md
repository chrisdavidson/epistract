...
3. **Each domain has a track record.** Scenario count is the maturity signal. Schema is static — a domain that's been run against six varied corpora has had its persona, epistemic rules, and entity-type coverage pressure-tested in ways one with zero scenarios hasn't. Drug-discovery has **six showcases** behind it (PICALM/Alzheimer's, KRAS G12C, Rare Disease, Immuno-Oncology, Cardiovascular, GLP-1 Competitive Intelligence — 111 documents collectively). Clinicaltrials launched with its first (GLP-1 Phase 3 Trials, 10 CT.gov protocols). Contracts and fda-product-labels are schema scaffolds awaiting their first showcase corpus. The Pre-built Domains table below tracks each domain's current scenario coverage.
