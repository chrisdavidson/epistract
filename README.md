...
**Think of it as a workspace, not a one-shot extractor.** Epistract is a [Claude Code](https://claude.ai/claude-code) plugin that turns the agentic terminal into a workspace where a scientific question becomes an executable workflow: identify the corpus needed to answer it (PubMed, Google Scholar, ClinicalTrials.gov, patent databases via SerpAPI, your own PDFs), select or extend a domain whose schema and persona match the analytical frame, build a graph specifically for that question, and analyze it through the persona-driven briefing and reactive chat. The graph that results is **bespoke to the question** — a different question, with a different corpus and possibly a different domain, produces a different graph. The framework's value is not in producing one canonical KG of biomedicine or contracts or anything else; it's in supporting the construction of many specialized KGs grounded to specific use cases. The scientist stays in the loop throughout — curating corpus, choosing domain, editing the persona if needed, asking follow-ups, refining.
