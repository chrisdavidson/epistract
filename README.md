...
> 📄 **Paper:** *Epistract: A Two-Layer Knowledge Graph Framework with an Epistemic Super-Domain Layer* — [paper/v2/main.pdf](paper/v2/main.pdf) (April 2026, framework v3.2.0).
...
**Think of it as a workspace, not a one-shot extractor — built for scientists and computational analysts.** Epistract is a [Claude Code](https://claude.ai/claude-code) plugin that turns the agentic terminal into an analytical workspace for biomedical researchers, clinical and regulatory analysts, competitive intelligence teams, and anyone whose work involves rigorous reading of large document corpora. It is **primarily an analysis tool**: it does not transact, does not write back to your corpus, does not generate new documents. It produces a structured graph, an epistemic classification, and an analyst briefing — all of which the scientist consumes. A scientific question becomes an executable workflow: identify the corpus needed to answer it (PubMed, Google Scholar, ClinicalTrials.gov, patent databases via SerpAPI, your own PDFs), select or extend a domain whose schema and persona match the analytical frame, build a graph specifically for that question, and analyze it through the persona-driven briefing and reactive chat. The graph that results is **bespoke to the question** — a different question, with a different corpus and possibly a different domain, produces a different graph. The scientist stays in the loop throughout — curating the corpus, choosing the domain, editing the persona if needed, asking follow-ups, refining.

**Iterative, not one-and-done.** As your understanding of a corpus grows, the framework supports iteration: ingest additional documents via `/epistract:ingest`; rebuild the epistemic classification with `/epistract:epistemic` to re-grade your evidence; refine the persona or hedging rules in your domain config; continue follow-up questions in the workbench chat. The framework itself does not auto-learn across runs — refinements are human-mediated for now, and [Issue #15](https://github.com/usathyan/epistract/issues/15) tracks the aspirational compounding mechanism. What does persist across runs is the scientist's broader context: Claude Code's own memory system and its plugin and MCP ecosystem (third-party data stores, external tools, project-level instructions in `CLAUDE.md`) keep what you've tried, what you've found, and what you want to try next available across sessions. The framework is the analytical primitive; Claude Code is the workspace layer around it.

**Where it fits, where to use export.** The design point is interactive analytical sessions — small-to-medium corpora that comfortably fit a Claude Code environment. The seven validated scenarios range from 10 to 34 documents per corpus; the framework has not been tested on corpora of hundreds or thousands of documents, and the upper bound at which the chat or briefing degrades is undetermined. For larger or persistent KGs, `/epistract:export` dumps the graph to GraphML, CSV, SQLite, or JSON for ingestion into Neo4j-class persistent stores. *Aspirational direction:* MCP-mediated data-source connectors that let the framework assemble small corpora on demand from PubMed, ClinicalTrials.gov, SerpAPI, and institutional sources — closing the loop between a scientific question and the bespoke corpus needed to answer it.

**It ships with four domains out of the box** (drug-discovery, contracts, clinicaltrials, fda-product-labels) and you can create a new one with `/epistract:domain` in under 15 minutes — a domain is 11–17 entity types and 10–22 relation types defined in YAML plus a persona string, no pipeline code changes. **Drug-discovery has been validated across six varied scenarios** (PICALM/Alzheimer's, KRAS G12C, Rare Disease, Immuno-Oncology, Cardiovascular, GLP-1 Competitive Intelligence — 111 documents collectively); clinicaltrials launched with its first (GLP-1 Phase 3 Trials, 10 CT.gov protocols). The Pre-built Domains table below tracks each domain's current scenario coverage — schema is static, but a domain that's been run against six varied corpora has had its persona, epistemic rules, and entity coverage pressure-tested in ways one with zero scenarios hasn't.
