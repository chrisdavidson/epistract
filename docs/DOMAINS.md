...
| **pharmacovigilance** | 12 / 12 | **0 scenarios** — a 16-report FAERS corpus is built at [`tests/corpora/09_pharmacovigilance/`](../tests/corpora/09_pharmacovigilance/), but no scenario write-up exists yet | Bradford-Hill causality vocabulary; MedDRA Preferred Term + WHO ATC nomenclature; reporter-type confidence calibration (HCP > consumer > lawyer); [`scripts/fetch_faers_corpus.py`](../scripts/fetch_faers_corpus.py) fetches reports from the FDA openFDA API | Adverse-event reports (FAERS / VAERS / MedWatch) — signal detection, dechallenge / rechallenge reasoning, causality assessment |
